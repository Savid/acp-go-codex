...
// native identity arrives once the app-server names the thread.
...
// install publishes a bound session under its ACP id.
...
// scheduleOpen defers the opening lifecycle snapshot behind the establishing
// response on a served connection, and emits it inline for an embedded host.
...
func (a *Agent) NewSession(ctx context.Context, params acp.NewSessionRequest) (resp acp.NewSessionResponse, err error) {
	...
	if err != nil {
		return acp.NewSessionResponse{}, s.startFailure(ctx, err)
	}

	s.id = acp.SessionId(thread.ID)
...
// store; a cold session is hydrated into the app-server's home and resumed.
func (a *Agent) restore(
	...
	s.images = slices.Clone(stored.record.Images)
	s.mode = firstNonEmpty(stored.record.Mode, modeDefault)
	...
	path, rows, err := a.hydrate(ctx, sessionID, stored, rt.home)
	if err != nil {
		return nil, nil, err
	}
	...
	if replay {
		if err := s.replay(ctx, rows, stored.record.Images); err != nil {
			_ = s.close(context.WithoutCancel(ctx))
			a.detach(ctx, s)
...
func (a *Agent) restoreActive(ctx context.Context, s *session, replay bool, release func()) (*session, func(), error) {
	...
	if err := s.replay(ctx, stored.rows, stored.record.Images); err != nil {
		release()
...
func inheritCarrier(meta sessionMeta, record sessionRecord) CodexOptions {
	...
	if options.ServiceTier == "" {
		options.ServiceTier = record.ServiceTier
	}

	if options.Personality == "" {
		options.Personality = record.Personality
	}

	if options.ApprovalPolicy == nil {
		options.ApprovalPolicy = wire.CloneValue(record.ApprovalPolicy)
	}

	if options.SandboxPolicy == nil {
		options.SandboxPolicy = wire.CloneValue(record.SandboxPolicy)
	}
...
func promptResult(resp acp.PromptResponse, err error, s *session) observer.PromptResult {
	result := observer.PromptResult{Err: err, Model: s.currentModel(), StopReason: string(resp.StopReason), Provider: "openai"}
...
// session with the same id. The rollout stays in Codex's home.
