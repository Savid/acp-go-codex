...
// install publishes a bound session under its thread id.
...
func (a *Agent) restore(
	...
	if !codex.ValidThreadID(string(sessionID)) {
...
func inheritCarrier(meta sessionMeta, record sessionRecord) CodexOptions {
	...
	if options.ApprovalPolicy == nil {
		options.ApprovalPolicy = cloneAny(record.ApprovalPolicy)
	}

	if options.SandboxPolicy == nil {
		options.SandboxPolicy = cloneAny(record.SandboxPolicy)
	}
