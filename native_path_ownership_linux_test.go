...
const (
	...
	nativeSharedIdentityRemedy       = "run the supervisor as root to isolate the agent identity, " +
...
// sharedNativeIdentityUID and sharedNativeIdentityGID stand in for a wrapper
// that never dropped privilege: the identity it runs as is the identity it
// hands native trees to, so nothing separates the two ends of the handoff.
const (
	sharedNativeIdentityUID = uint32(1000)
	sharedNativeIdentityGID = uint32(1000)
)
...
// identity is also the target identity. No privilege boundary separates the
// wrapper from the identity it hands the tree to in that shape, so the
// root-owned directories every path is reached through are acceptable ancestors
// — and nothing else is: a third identity's ancestor, an ancestor root left
// writable without sticky protection, an ancestor that identity cannot enter,
// and a generated root root still owns are all refused.
func TestGeneratedNativeAncestorUnderASharedIdentityAcceptsOnlyRootAncestors(t *testing.T) {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			name: "ancestor owned by a third identity",
			stat: directory(0o755, 4242, 4242),
			want: "generated native path ancestor is uid=4242 gid=4242; " + nativeSharedIdentityRemedy,
		},
		{
		...
		t.Run(testCase.name, func(t *testing.T) {
			require.ErrorContains(t, validateGeneratedNativeAncestor(
				...
				sharedNativeIdentityUID, sharedNativeIdentityGID,
				sharedNativeIdentityUID, sharedNativeIdentityGID,
			), testCase.want)
		})
	}
	...
	for _, accepted := range []struct {
		...
		{stat: directory(0o1777, 0, 0)},
		{stat: directory(0o711, sharedNativeIdentityUID, sharedNativeIdentityGID)},
		{stat: directory(0o700, sharedNativeIdentityUID, sharedNativeIdentityGID), final: true},
	} {
		require.NoError(t, validateGeneratedNativeAncestor(
	...
	// Two distinct identities keep the rule they had: a root-owned ancestor is
	// trusted only because the wrapper is root, and a refusal names nothing an
	// operator could change about a boundary that is working as intended.
	isolated := validateGeneratedNativeAncestor(
		directory(0o755, 0, 0), false, 4242, 4242, nativeOwnershipTargetUID, nativeOwnershipTargetGID,
	)
	require.ErrorContains(t, isolated, nativeOwnershipUntrustedAncestry)
	require.NotContains(t, isolated.Error(), nativeSharedIdentityRemedy)
}
...
// owner rights, and only its ancestry admits the root-owned components a home
...
func TestDurableNativeAncestorUnderASharedIdentityAcceptsOnlyRootAncestors(t *testing.T) {
	...
	for _, testCase := range []struct {
	...
	}{
		{
			name: "ancestor owned by a third identity",
			stat: directory(0o755, 4242, 4242),
			want: "native-owned path ancestor is uid=4242 gid=4242; " + nativeSharedIdentityRemedy,
		},
		{
			name: "ancestor owned by root with a foreign group",
			stat: directory(0o755, 0, 4242),
	...
	// Two distinct identities keep the rule they had, including the refusal
	// wording, which stays free of advice about a boundary that is intact.
	isolated := validateDurableNativeAncestor(
		directory(0o755, 0, 0), false, 4242, 4242, nativeOwnershipTargetUID, nativeOwnershipTargetGID,
	)
	require.ErrorContains(t, isolated, "native-owned path ancestor is uid=0 gid=0")
	require.NotContains(t, isolated.Error(), nativeSharedIdentityRemedy)
}
...
// presents: its own identity is the native identity, and the tree it was handed
// hangs from root-owned directories it will never own. The effective identity is
...
func TestNativeOwnershipWalksARootOwnedAncestryUnderASharedIdentity(t *testing.T) {
	...
	const (
		sharedUID = nativeOwnershipTargetUID
		sharedGID = nativeOwnershipTargetGID
	)

	parent := testTraversableTempDir(t)
	mid := filepath.Join(parent, "mid")
	require.NoError(t, os.Mkdir(mid, 0o755))
	require.NoError(t, os.Chmod(mid, 0o755))

	native := filepath.Join(mid, "native")
	require.NoError(t, os.Mkdir(native, 0o700))
	...
	require.NoError(t, os.Chown(mid, 4242, 4242))
	...
	require.ErrorContains(t, handoffErr, nativeSharedIdentityRemedy)
	...
	require.ErrorContains(t, validationErr, nativeSharedIdentityRemedy)

	// Restoring the wrapper's own root identity restores the isolated rule: the
	// same third-party ancestor is refused, and the refusal offers no advice.
	effectiveUIDSource, effectiveGIDSource = realUID, realGID

	isolatedErr := validateNativeOwnedDirectory(native, isolation)
	require.ErrorContains(t, isolatedErr, "native-owned path ancestor is uid=4242 gid=4242")
