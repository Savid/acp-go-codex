...
// read that disagrees with the first.
var (
...
// remaining answers are to give the supervisor one, or to anchor the tree on a path
// the identity already owns.
...
func handoffNativeOwnershipDirectory(
	...
	for _, entry := range entries {
		name := entry.Name()
		if name == "." || name == handoffParentName || strings.ContainsRune(name, '/') {
