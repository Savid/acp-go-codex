...
func TestSessionIDErrors(t *testing.T) {
	...
	if _, err := NewAgent().NewSession(ctx, NewSessionRequest(absTestPath("tmp", "project"))); err == nil {
		t.Fatal("NewSession ignored session id generation failure")
	}
	forkIDAgent := NewAgent()
	if err := forkIDAgent.storeStartedSession(newSession(forkIDAgent, "parent-id", absTestPath("tmp", "project"), nil, codex.Thread{ID: "parent-thread"}, newSpyCodexClient(), sessionMeta{}, nil)); err != nil {
		t.Fatalf("store fork id parent: %v", err)
	}
	raw, err := json.Marshal(ForkSessionRequest("parent-id", absTestPath("tmp", "project")))
	if err != nil {
