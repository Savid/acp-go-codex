...
func (s *session) openStream(ctx context.Context, rt *runtime) error {
	...
	return s.lc.Open(ctx, lifecycle.NewIncarnation(string(s.id)), s.lifecycleNegotiated(), s.deliverLifecycle)
}
...
func (s *session) lcIdle(ctx context.Context, c *cycle, verdict cycleVerdict) error {
	return s.lc.Idle(ctx, c.Cycle, verdict.stopReason, verdict.outcome)
}
