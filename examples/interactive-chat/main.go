...
func run(ctx context.Context, args []string, stdin io.Reader, stdout io.Writer, stderr io.Writer) int {
	...
	model := flags.String("model", "", "model for the session")
	...
	go func() {
		served <- codexacp.Serve(serveCtx, agentReader, agentWriter, codexacp.WithLogger(slog.New(slog.DiscardHandler)))
	}()
...
func chat(ctx context.Context, conn agentConnection, input *bufio.Reader, cwd string, model string, stdout io.Writer) error {
	...
	var opts []codexacp.SessionRequestOption
	if model != "" {
		opts = append(opts, codexacp.WithSessionCodexOptions(codexacp.NewCodexOptions(codexacp.WithCodexModel(model))))
	}

	session, err := conn.NewSession(ctx, codexacp.NewSessionRequest(cwd, opts...))
	if err != nil {
		return err
	}
	...
	for {
		...
		if _, err := conn.Prompt(ctx, codexacp.TextPromptRequest(session.SessionId, text)); err != nil {
			return err
		}
