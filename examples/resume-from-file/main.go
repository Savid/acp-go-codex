...
func run(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer) int {
	...
	go func() {
		served <- codexacp.Serve(serveCtx, agentReader, agentWriter, codexacp.WithSessionStore(store), codexacp.WithLogger(slog.New(slog.DiscardHandler)))
	}()
...
func converse(ctx context.Context, conn agentConnection, cwd string, sessionID acp.SessionId, prompt string) (acp.SessionId, error) {
	...
	} else if _, err := conn.ResumeSession(ctx, codexacp.ResumeSessionRequest(sessionID, cwd)); err != nil {
		return "", err
	}
	...
	if _, err := conn.Prompt(ctx, codexacp.TextPromptRequest(sessionID, prompt)); err != nil {
		return "", err
	}
