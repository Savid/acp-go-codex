...
var buildIntegrationBinary = sync.OnceValues(func() (string, error) {
	dir, err := os.MkdirTemp("", "acp-go-codex-integration-")
	if err != nil {
		return "", err
	}
	binaryBuildDir = dir
	...
	defer cancel()
	cmd := exec.CommandContext(ctx, "go", "build", "-o", binary, "./cmd/acp-go-codex")
...
func integrationBinaryPath(t *testing.T) string {
	t.Helper()
	if binary := os.Getenv("ACP_GO_CODEX_AGENT_BINARY"); binary != "" {
