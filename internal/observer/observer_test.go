...
func TestObserverRecordsACPRequestsAndProcessStarts(t *testing.T) {
	...
	observer.RecordCodexProcessStart(ctx)
	ctx, finishPrompt := observer.StartPrompt(ctx, map[string]any{}, "gpt-test")
	...
	if got := int64MetricSum(metrics, "acp_go_codex.acp.request.count"); got != 2 {
		t.Fatalf("request count = %d, want 2", got)
	}
	if got := int64MetricSum(metrics, "acp_go_codex.codex.process.start.count"); got != 1 {
		t.Fatalf("process start count = %d, want 1", got)
	}
	if got := histogramMetricCount(metrics, "acp_go_codex.acp.request.duration"); got != 2 {
		t.Fatalf("request duration count = %d, want 2", got)
	}
	if got := int64MetricSum(metrics, "acp_go_codex.session.prompt.count"); got != 2 {
		t.Fatalf("prompt count = %d, want 2", got)
	}
	if got := int64MetricSum(metrics, "acp_go_codex.session.cancel.count"); got != 1 {
		t.Fatalf("prompt cancel count = %d, want 1", got)
	}
	if got := int64MetricSum(metrics, "acp_go_codex.session.active"); got != 0 {
		t.Fatalf("active session sum = %d, want 0", got)
	}
	if got := histogramMetricCount(metrics, "acp_go_codex.session.prompt.duration"); got != 2 {
	...
	nilObserver.RecordCodexProcessStart(ctx)
