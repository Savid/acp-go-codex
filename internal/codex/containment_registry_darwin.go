...
const (
	darwinRegistryDir        = "acp-go-codex-containment"
...
func createDarwinRecord(parent string, record darwinContainmentRecord) error {
	...
	for _, entry := range entries {
		...
		if now.Sub(info.ModTime()) > darwinRecordLifetime {
			...
			expiredRecord, readErr := readCurrentDarwinRecord(parent, recordPath, entry.Name())
			if readErr != nil {
...
func CleanupDarwinContainment(scratchParent, runtimeID string, force bool, output io.Writer) error { //nolint:gocyclo // The fixed TERM/KILL/deadline ladder keeps every terminal check explicit.
	...
	deadline := darwinCleanupNow().Add(supervisorQuiesceWindow)
...
func readDarwinRecords(scratchParent, runtimeID string) (string, []darwinContainmentRecord, error) {
	...
	for _, entry := range entries {
		...
		record, recordErr := readCurrentDarwinRecord(parent, filepath.Join(registry, entry.Name()), entry.Name())
		if recordErr != nil {
...
func readCurrentDarwinRecord(parent, recordPath, entryName string) (darwinContainmentRecord, error) {
	info, infoErr := darwinRegistryLstat(recordPath)
	if infoErr != nil {
		return darwinContainmentRecord{}, fmt.Errorf("inspect containment record: %w", infoErr)
	}

	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 || info.Mode().Perm() != 0o600 {
		return darwinContainmentRecord{}, fmt.Errorf("containment record %s is not a mode-0600 regular file", entryName)
	}

	fd, openErr := darwinRegistryOpen(recordPath, unix.O_RDONLY|unix.O_NOFOLLOW, 0)
	if openErr != nil {
		return darwinContainmentRecord{}, openErr
	}
	...
	if statErr != nil || !openedInfo.Mode().IsRegular() || openedInfo.Mode().Perm() != 0o600 {
		...
		return darwinContainmentRecord{}, errors.Join(fmt.Errorf("containment record %s is not a mode-0600 regular file", entryName), statErr)
	}
	...
	if !validCurrentDarwinRecord(parent, strings.TrimSuffix(entryName, ".json"), record) {
		return darwinContainmentRecord{}, fmt.Errorf("invalid current-format containment record %s", entryName)
	}
...
func validDarwinLifecycle(lifecycle string) bool {
	switch lifecycle {
	case lifecycleRuntime, "session", darwinLifecyclePrompt, lifecycleDiscovery:
		return true
	default:
		return false
	}
...
func validDarwinGenerationPrefix(root string) bool {
	...
	return strings.HasPrefix(base, "acp-go-codex-runtime-") || strings.HasPrefix(base, "acp-go-codex-account-")
}
