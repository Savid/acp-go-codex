...
func TestDarwinGenerationRecordCurrentFormatAndDistinctIdentity(t *testing.T) {
	parent := t.TempDir()
	root1 := filepath.Join(parent, "acp-go-codex-runtime-one")
	root2 := filepath.Join(parent, "acp-go-codex-runtime-two")
...
func TestDarwinRegistrySerializesConcurrentReplacement(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-concurrent")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestDarwinDiagnoseIsReadOnly(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-diagnose")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestDarwinCleanupRequiresSelectionAndSignalsFreshIndividualCandidates(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-cleanup")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
...
func TestDarwinCleanupDoesNotReportESRCHAsSignaled(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-esrch")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
...
func TestDarwinCleanupScanDeadlineTerminalizesWithoutLateOperations(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-scan-deadline")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestDarwinCleanupRetainsReplacementGenerationRoot(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-replaced")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestDarwinCleanupNeverRemovesRootCreatedAfterMissingValidation(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-created-late")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
...
func TestDarwinCleanupDeadlineIsTerminalAtEveryEmptyCandidateStage(t *testing.T) {
	...
	for _, expireAt := range []int{2, 3, 4, 7, 8, 9, 10} {
		t.Run(strconv.Itoa(expireAt), func(t *testing.T) {
			parent := t.TempDir()
			root := filepath.Join(parent, "acp-go-codex-runtime-deadline-stage")
			if err := os.Mkdir(root, 0o700); err != nil {
				t.Fatal(err)
			}
			generation, err := NewDarwinGenerationRecord(parent, root, "runtime")
...
func TestDarwinCleanupRootRemovalConsumesBudget(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-removal-deadline")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "discovery")
...
func TestDarwinCleanupDeadlineIsTerminalAfterEachSignalStage(t *testing.T) {
	...
	for _, stage := range []string{"initial-term", "pre-kill-term", "pre-kill-term-grace", "kill"} {
		t.Run(stage, func(t *testing.T) {
			parent := t.TempDir()
			root := filepath.Join(parent, "acp-go-codex-runtime-signal-stage")
			if err := os.Mkdir(root, 0o700); err != nil {
				t.Fatal(err)
			}
			generation, err := NewDarwinGenerationRecord(parent, root, "runtime")
...
func TestDarwinCleanupRejectsRootOutsideParent(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-root")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
	...
	record.GenerationRoot = filepath.Join(outsideParent, "acp-go-codex-runtime-outside")
...
func TestDarwinCleanupRejectsMissingIncompleteRoot(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-missing")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
...
func TestDarwinRecordValidationRejectsMalformedCurrentFormat(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "relative root", mutate: func(record *darwinContainmentRecord) { record.GenerationRoot = "acp-go-codex-runtime-relative" }},
		{name: "partial child", mutate: func(record *darwinContainmentRecord) { pid := 1; record.DirectChildPID = &pid }},
	} {
		t.Run(test.name, func(t *testing.T) {
			parent := t.TempDir()
			root := filepath.Join(parent, "acp-go-codex-runtime-record")
			if err := os.Mkdir(root, 0o700); err != nil {
				t.Fatal(err)
			}
			generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
...
func TestDarwinRecordReaderRejectsUnsafeFilesystemObjects(t *testing.T) {
	t.Run("record mode", func(t *testing.T) {
		parent := t.TempDir()
		root := filepath.Join(parent, "acp-go-codex-runtime-mode")
		if err := os.Mkdir(root, 0o700); err != nil {
			t.Fatal(err)
		}
		generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
	...
	t.Run("lock symlink", func(t *testing.T) {
		...
		root := filepath.Join(parent, "acp-go-codex-runtime-lock")
		if err := os.Mkdir(root, 0o700); err != nil {
			t.Fatal(err)
		}
...
func TestDarwinCleanupReportsAmbiguousIdentityAndRetainsRoot(t *testing.T) {
	parent := t.TempDir()
	root := filepath.Join(parent, "acp-go-codex-runtime-ambiguous")
	if err := os.Mkdir(root, 0o700); err != nil {
		t.Fatal(err)
	}
	generation, err := NewDarwinGenerationRecord(parent, root, "prompt")
