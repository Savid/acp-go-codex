...
func TestRunGuardianRecoveryMarkerAndExitBranches(t *testing.T) {
	t.Run("pre-readiness marker", func(t *testing.T) {
		...
		require.NoError(t, writeNativePID(config.NativePIDFile, 123))
		supervisorExecutable = func() (string, error) { return "/usr/bin/true", nil }
		supervisorExecCommand = func(string, ...string) *exec.Cmd { return exec.Command("/usr/bin/true") }
	...
	for _, test := range []struct {
		...
		{name: "post-readiness marker", exit: "0"},
		{name: "post-readiness exit failure", exit: "7", precompleted: true, wantErrorText: "liveness supervisor exited"},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			supervisorExecCommand = func(string, ...string) *exec.Cmd {
				return exec.Command("/bin/sh", "-c", "echo '"+supervisorReadyPrefix+`{"nativePid":123}`+"' >&2; exit "+test.exit)
			}
			supervisorInput = strings.NewReader("")
			supervisorOutput = io.Discard
			supervisorError = io.Discard
...
func TestRunLivenessControlEOFAndNativeFailure(t *testing.T) {
	...
	config := supervisorConfig{
		...
		Home: filepath.Join(root, "home"), Scratch: root, ScratchParent: filepath.Dir(root), LifecycleKind: "runtime", DarwinBestEffort: true,
		Started: filepath.Join(root, "started"), Completion: filepath.Join(root, "complete"),
		NativePIDFile: filepath.Join(root, "native.pid"),
	}
