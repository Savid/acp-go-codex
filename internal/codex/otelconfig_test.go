...
func TestCodexOTELConfigDisabledByDefault(t *testing.T) {
	config, err := OTELConfigFromEnv(map[string]string{
		"OTEL_SERVICE_NAME": "ignored-for-codex-native",
	})
	if err != nil {
		t.Fatalf("OTELConfigFromEnv returned error: %v", err)
	}
...
func TestCodexOTELGenericEndpointUsedVerbatimForGRPC(t *testing.T) {
	config, err := OTELConfigFromEnv(map[string]string{
		"OTEL_TRACES_EXPORTER":      "otlp",
		otelEnvExporterOTLPProtocol: "grpc",
		otelEnvExporterOTLPEndpoint: "http://collector:4317",
	})
	if err != nil {
		t.Fatalf("generic grpc endpoint config returned error: %v", err)
	}
	args := strings.Join(config.ExtraArgs, " ")
	requireContainsAll(t, args,
		`otel.trace_exporter="otlp-grpc"`,
		`otel.trace_exporter.otlp-grpc.endpoint="http://collector:4317"`,
	)
}
...
func TestCodexOTELInvalidProtocol(t *testing.T) {
	_, err := OTELConfigFromEnv(map[string]string{
		"OTEL_TRACES_EXPORTER":               "otlp",
