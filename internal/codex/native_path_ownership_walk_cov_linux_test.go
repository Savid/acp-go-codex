...
// that never dropped privilege: the identity it runs as is the identity it hands
// native trees to, so nothing separates the two ends of the handoff.
const (
...
// target identity. No privilege boundary separates the wrapper from the identity
// it hands the tree to in that shape, so the root-owned directories every path is
// reached through are acceptable ancestors — and nothing else is: a third
// identity's ancestor, an ancestor root left writable without sticky protection,
// an ancestor that identity cannot enter, and a generated root root still owns
// are all refused.
...
// TestGeneratedNativeTreeWalksARootOwnedAncestryUnderASharedIdentity proves the
// whole handoff accepts the shape a wrapper that never dropped privilege
// presents: its own identity is the identity the tree is handed to, and the tree
// hangs from root-owned directories that identity will never own. The effective
// identity is staged through its seams so the proof does not depend on which
// identity runs the tests; the chowns behind the handoff still need the root the
// rest of this file requires.
func TestGeneratedNativeTreeWalksARootOwnedAncestryUnderASharedIdentity(t *testing.T) {
	native, parent := nativeOwnershipGeneratedRoot(t)
	...
	handoffErr := handoffGeneratedNativeTree(native, nativeOwnershipIsolation())
	...
	isolatedErr := handoffGeneratedNativeTree(native, nativeOwnershipIsolation())
	require.ErrorContains(t, isolatedErr, nativeOwnershipUntrustedAncestry)
