...
func TestLifecycleMetaRejectionsCarryTheUniformUnsupportedShape(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		"effort is empty":                 {meta: options(map[string]any{metaEffortKey: ""}), field: "_meta.codex.options.effort"},
		"personality is not a string":     {meta: options(map[string]any{metaPersonalityKey: 42}), field: "_meta.codex.options.personality"},
		"personality is empty":            {meta: options(map[string]any{metaPersonalityKey: ""}), field: "_meta.codex.options.personality"},
