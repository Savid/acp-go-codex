...
func newHarness(t *testing.T, extra ...Option) *harness {
	...
	h := &harness{t: t, conn: conn, rec: rec, input: input, cancel: cancel, served: served, home: applyOptions(options).Home}
...
// rawEventItem reports whether a raw-event notification for the named native
// item has been delivered.
func rawEventItem(h *harness, itemID string) bool {
	h.rec.mu.Lock()
	...
	for _, raw := range h.rec.raw {
		...
		if err := json.Unmarshal(raw, &payload); err == nil && payload.Event.ItemID == itemID {
			return true
		}
	}
